...
class FlowDiscoveryAgent:
    ...
    def discover(self, url):
        ...
        # Build prompt with actual page content if available
        if page_content:
            ...
            prompt = f"""I have extracted the following content from the webpage at {url}. Based on this actual page content, identify the most important user flows (login, signup, checkout, search, etc.).
...
{page_content.get('visible_text', '')[:3000]}

HTML Structure (key elements):
{page_content.get('html_structure', '')[:5000]}
...
Respond with ONLY the JSON array, no additional text or explanation."""
            ...
            print(
                f"[DEBUG] Using page_content-based prompt (length: {len(prompt)})")
        else:
